...
# ---------------------------------------------------------------------------
# Вход в маркетплейс от имени автора
# ---------------------------------------------------------------------------


class BloggerImpersonateResponse(BaseModel):
    """Одноразовая ссылка входа в маркетплейс от имени автора.

    ``url`` ведёт на SSO-callback маркетплейса и обменивает код на сессию автора;
    ссылка срабатывает один раз и живёт ``expires_in`` секунд.
    """
