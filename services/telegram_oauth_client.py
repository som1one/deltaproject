...
class HttpxJWKClient(PyJWKClient):
    def fetch_data(self) -> Any:
        ...
        try:
            ...
            raise PyJWKClientError(f"AsyncClient fetch failed: {type(e).__name__} {e}") from e
...
async def exchange_code(*, code: str, redirect_uri: str) -> str:
    ...
    try:
        ...
        raise TelegramOAuthError(f"Не удалось связаться с Telegram OAuth: {type(exc).__name__} {exc}") from exc
